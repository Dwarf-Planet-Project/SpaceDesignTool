    ...
    : QDialog(parent), d_plot(g), d_widget(NULL), d_widget_type(wt)
{
	...
	if (wt == Tex){
		setWindowTitle(tr("Analysis") + " - " + tr("Tex Equation Editor"));
        ...
        setWindowTitle(tr("Analysis") + " - " + tr("Window Geometry"));
	else
		setWindowTitle(tr("Analysis") + " - " + tr("Object Properties"));
...
void EnrichmentDialog::updateForm(bool error)
{
    ...
    if (!error) {
		...
		QMessageBox::critical((QWidget *)parent(), tr("Analysis") + " - " + tr("Network connection error"),
