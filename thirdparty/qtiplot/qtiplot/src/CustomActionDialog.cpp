...
CustomActionDialog::CustomActionDialog(QWidget* parent, Qt::WFlags fl)
    : QDialog(parent, fl)
{
    setWindowTitle(tr("Analysis") + " - " + tr("Add Custom Action"));
...
bool CustomActionDialog::validUserInput()
{
	...
	if (textBox->text().isEmpty()){
        QMessageBox::critical(app, tr("Analysis") + " - " + tr("Error"),
...
void CustomActionDialog::removeAction()
{
    ...
    if (QMessageBox::Yes != QMessageBox::question(this, tr("Analysis") + " - " + tr("Remove Action"), s, QMessageBox::Yes, QMessageBox::Cancel))
        return;
...
void CustomActionDialog::saveAction(QAction *action)
{
	...
	if (!f.open( QIODevice::WriteOnly)){
		...
		QMessageBox::critical(app, tr("Analysis") + " - " + tr("File Save Error"),
