...
--------------------------------------------------------------------
    ...
    This file has been extensive patched by Guillermo as to accomodate the
    required changes to load the main window from the STA analysis function
...
#include "3rdparty/qtassisstant/qassistantclient.h"
...
{
    setWindowTitle(tr("STA analysis - untitled"));
    //setObjectName("STA Analysis");
        initGlobalConstants();
...
void ApplicationWindow::insertTranslatedStrings()
{
        ...
        setWindowTitle(tr("Analysis - untitled"));
        ...
        if (scriptWindow)
        scriptWindow->setWindowTitle(tr("Analysis - Script Window"));
        explorerWindow->setWindowTitle(tr("Project Explorer"));
        logWindow->setWindowTitle(tr("Results Log"));
        undoStackWindow->setWindowTitle(tr("Undo Stack"));
#ifdef SCRIPTING_CONSOLE
...
void ApplicationWindow::plot3DBars()
{
    ...
    if (!w)
        ...
        if (w->inherits("Table")){
            ...
            QMessageBox::warning(this, tr("Analysis - Plot error"),tr("You must select exactly one column for plotting!"));
        }
...
void ApplicationWindow::plotPie()
{
    ...
    if (!table)
        ...
        if(table->selectedColumns().count() != 1){
        QMessageBox::warning(this, tr("Analysis - Plot error"),
        ...
        QMessageBox::warning(this, tr("Analysis - Error"), tr("Please select a column to plot!"));
}
...
void ApplicationWindow::plotVectXYXY()
{
    ...
    if (!table)
        ...
        QMessageBox::warning(this, tr("Analysis - Error"), tr("Please select four columns for this operation!"));
}
...
{
        if (!hasTable()){
        QMessageBox::warning(this,tr("Analysis - Warning"),
        ...
        {
        QMessageBox::critical(this,tr("Analysis - Warning"),
    ...
    ad->setWindowTitle(tr("Analysis - Choose data set"));
...
void ApplicationWindow::change3DMatrix()
{
    ...
    ad->setWindowTitle(tr("Analysis - Choose matrix to plot"));
...
void ApplicationWindow::exportMatrix(const QString& exportFilter)
{
        ...
        QMessageBox::critical(this, tr("Analysis - Export error"),
...
Matrix* ApplicationWindow::importImage(const QString& fileName, bool newWindow)
{
        ...
        fn = QFileDialog::getOpenFileName(this, tr("Analysis - Import image from file"), imagesDirPath, filter);
                if ( !fn.isEmpty() ){
                        QFileInfo fi(fn);
...
void ApplicationWindow::loadImage()
{
    ...
    QString fn = QFileDialog::getOpenFileName(this, tr("Analysis - Load image from file"), imagesDirPath, filter);
        if ( !fn.isEmpty() ){
                loadImage(fn);
                QFileInfo fi(fn);
                imagesDirPath = fi.dirPath(true);
...
{//used when plotting from the panel menu
    ...
    if (!t)
        ...
        if((int)list.count() < 1) {
        QMessageBox::warning(this, tr("Analysis - Plot error"), tr("Please select a Y column to plot!"));
                return 0;
        }
...
Table* ApplicationWindow::newTable(const QString& caption, int r, int c)
{
        ...
        if (w->objectName() != caption){//the table was renamed
            ...
            QMessageBox:: warning(this, tr("Analysis - Renamed Window"),
...
void ApplicationWindow::addErrorBars()
{
    ...
    if (!g->curveCount()){
        QMessageBox::warning(this, tr("Analysis - Warning"), tr("There are no curves available on this plot!"));
                return;
        }

        if (g->isPiePlot()){
        QMessageBox::warning(this, tr("Analysis - Warning"), tr("This functionality is not available for pie plots!"));
        return;
        }
...
void ApplicationWindow::defineErrorBars(const QString& name, int type, const QString& percent, int direction)
{
    ...
    if (!w)
        ...
        QMessageBox::critical(this,tr("Analysis - Error bars error"),
                ...
                case OpenProjectDialog::NewProject:
                        {
                                ...
                                if (projectname != "untitled"){
                                        ...
                                        if (fn == pn){
                        QMessageBox::warning(this, tr("Analysis - File openning error"),
                                ...
                                if (!fi.exists ()){
                    QMessageBox::critical(this, tr("Analysis - File openning error"),
...
ApplicationWindow* ApplicationWindow::open(const QString& fn, bool factorySettings, bool newProject)
{
    ...
    QStringList lst = s.split(QRegExp("\\s"), QString::SkipEmptyParts);
        bool qtiProject = (lst.count() < 2 || lst[0] != "Analysis") ? false : true;
        if (!qtiProject){
                if (QFile::exists(fname + "~")){
            int choice = QMessageBox::question(this, tr("Analysis - File opening error"),
            ...
            if (choice == QMessageBox::Yes)
                ...
                QMessageBox::critical(this, tr("Analysis - File opening error"),  tr("The file: <b> %1 </b> was not created using QtiPlot!").arg(fn));
            return 0;
                }
...
void ApplicationWindow::openRecentProject(int index)
{
        ...
        QFile f(fn);
        if (!f.exists()){
        QMessageBox::critical(this, tr("Analysis - File Open Error"),
...
ApplicationWindow* ApplicationWindow::openProject(const QString& fn, bool factorySettings, bool newProject)
{
        ...
        app->setWindowTitle(tr("Analysis") + " - " + fn);
        ...
        if (list[0] == "<scripting-lang>")
        {
            ...
            QMessageBox::warning(app, tr("Analysis - File opening error"),
    ...
    progress.setWindowTitle(tr("Analysis - Opening file") + ": " + baseName);
...
void ApplicationWindow::scriptError(const QString &message, const QString &scriptName, int lineNumber)
{
        ...
        QMessageBox::critical(this, tr("Analysis") + " - "+ tr("Script Error"), message);
}
...
void ApplicationWindow::restartScriptingEnv()
{
        ...
        QMessageBox::critical(this, tr("Analysis - Scripting Error"),
...
void ApplicationWindow::openTemplate()
{
    ...
    QString fn = QFileDialog::getOpenFileName(this, tr("Analysis - Open Template File"), templatesDir, filter);
        if (!fn.isEmpty()){
                QFileInfo fi(fn);
            ...
            QMessageBox::critical(this,tr("Analysis - File opening error"),
...
{
#ifdef Q_OS_MAC // Mac
        QSettings settings(QSettings::IniFormat,QSettings::UserScope, "ProIndependent", "Analysis");
#else
        QSettings settings(QSettings::NativeFormat,QSettings::UserScope, "ProIndependent", "Analysis");
#endif
...
void ApplicationWindow::exportAllGraphs()
{
    ...
    ied->selectFilter(d_image_export_filter);
        ...
        foreach(MdiSubWindow *w, windows){
            ...
            QMessageBox msgBox(QMessageBox::Question, tr("Analysis - Overwrite file?"), msg,
...
bool ApplicationWindow::saveProject(bool compress)
{
    ...
    setWindowTitle("Analysis - " + projectname);
        savedProject();
...
bool ApplicationWindow::setWindowName(MdiSubWindow *w, const QString &text)
{
        ...
        newName.replace("-", "_");
        if (newName.isEmpty()){
        QMessageBox::critical(this, tr("Analysis - Error"), tr("Please enter a valid name!"));
                return false;
        ...
        QMessageBox::critical(this, tr("Analysis - Error"), tr("Name <b>%1</b> already exists!").arg(newName)+
                                "<p>"+tr("Please choose another name!")+
                ...
                foreach(MdiSubWindow *w, windows){
                        if (w->inherits("Table") || w->isA("Matrix")){
                                ...
                                if (f.exists(fileName) && confirmOverwrite){
                                        QApplication::restoreOverrideCursor();
                    switch(QMessageBox::question(this, tr("Analysis - Overwrite file?"),
...
void ApplicationWindow::showRowsDialog()
{
        ...
        bool ok;
    int rows = QInputDialog::getInteger(this, tr("Analysis - Enter rows number"), tr("Rows"),
...
void ApplicationWindow::showDeleteRowsDialog()
{
    Table *t = (Table*)activeWindow(TableWindow);
        ...
        bool ok;
    int start_row = QInputDialog::getInteger(this, tr("Analysis - Delete rows"), tr("Start row"),
                    1, 1, t->numRows(), 1, &ok);
    if (ok){
        int end_row = QInputDialog::getInteger(this, tr("Analysis - Delete rows"), tr("End row"),
...
void ApplicationWindow::showColsDialog()
{
        ...
        bool ok;
    int cols = QInputDialog::getInteger(this, tr("Analysis - Enter columns number"), tr("Columns"),
...
void ApplicationWindow::showColumnValuesDialog()
{
    ...
    if (w->selectedColumns().count()>0 || w->table()->currentSelection() >= 0){
        ...
        QMessageBox::warning(this, tr("Analysis - Column selection error"), tr("Please select a column first!"));
}
...
void ApplicationWindow::sortActiveTable()
{
        ...
        QMessageBox::warning(this, "Analysis - Column selection error","Please select a column first!");
}
...
void ApplicationWindow::correlate()
{
        ...
        if ((int)s.count() != 2){
        QMessageBox::warning(this, tr("Analysis - Error"), tr("Please select two columns for this operation!"));
                return;
        }
...
void ApplicationWindow::autoCorrelate()
{
        ...
        if ((int)s.count() != 1)
        {
        QMessageBox::warning(this, tr("Analysis - Error"), tr("Please select exactly one columns for this operation!"));
                return;
        }
...
void ApplicationWindow::showRowStatistics()
{
        ...
        QMessageBox::warning(this, tr("Analysis - Row selection error"),
...
void ApplicationWindow::showColumnOptionsDialog()
{
        ...
        if(t->selectedColumns().count()>0) {
                ...
                QMessageBox::warning(this, tr("Analysis"), tr("Please select a column first!"));
}
...
void ApplicationWindow::removePoints()
{
        ...
        {
                switch(QMessageBox::warning (this,tr("Analysis"),
...
void ApplicationWindow::showRangeSelectors()
{
        ...
        if (!plot)
                return;
        if (plot->isEmpty()){
        QMessageBox::warning(this, tr("Analysis - Warning"), tr("There are no plot layers available in this window!"));
                btnPointer->setChecked(true);
                return;
        }
...
void ApplicationWindow::addImage()
{
    ...
    QString fn = QFileDialog::getOpenFileName(this, tr("Analysis - Insert image from file"), imagesDirPath, filter);
        if ( !fn.isEmpty() ){
                QFileInfo fi(fn);
                imagesDirPath = fi.dirPath(true);
...
MdiSubWindow* ApplicationWindow::clone(MdiSubWindow* w)
{
    if (!w) {
        w = activeWindow();
                if (!w){
            QMessageBox::critical(this,tr("Analysis - Duplicate window error"),
        ...
        ((Table *)nw)->copy(t);
                ...
                if (!g->hasData()){
                QApplication::restoreOverrideCursor();
            QMessageBox::warning(this, tr("Analysis - Duplicate error"), tr("Empty 3D surface plots cannot be duplicated!"));
                return 0;
        }
...
void ApplicationWindow::showMoreWindows()
{
        if (explorerWindow->isVisible())
                QMessageBox::information(this, "Analysis",tr("Please use the project explorer to select a window!"));
        else
                explorerWindow->show();
}
...
void ApplicationWindow::closeEvent( QCloseEvent* ce )
{
        ...
        if (!saved){
                ...
                switch( QMessageBox::information(this, tr("Analysis"), s, tr("Yes"), tr("No"),
                                        tr("Cancel"), 0, 2 ) ){
                        case 0:
                                if (!saveProject()){
                                        ce->ignore();
...
void ApplicationWindow::chooseHelpFolder()
{
        ...
        if (!dir.isEmpty()){
                ...
                if (!helpFile.exists()){
            QMessageBox::critical(this, tr("Analysis - index.html File Not Found!"),
...
void ApplicationWindow::showStandAloneHelp()
{
        ...
        {
        QMessageBox::critical(0, tr("Analysis - Help Files Not Found!"),
        ...
        {
        QMessageBox::critical(0, tr("Analysis - Help Profile Not Found!"),
...
void ApplicationWindow::showHelp()
{
        ...
        if (!helpFile.exists())
        {
        QMessageBox::critical(this,tr("Analysis - Help Files Not Found!"),
        ...
        if (!QFile(profilePath).exists())
        {
        QMessageBox::critical(this,tr("Analysis - Help Profile Not Found!"),
                                tr("The assistant could not start because the file <b>%1</b> was not found in the help file directory!").arg("qtiplot.adp")+"<br>"+
                                tr("This file is provided with the QtiPlot manual which can be downloaded from the following internet address:")+
                                "<p><a href = http://soft.proindependent.com/manuals.html>http://soft.proindependent.com/manuals.html</a></p>");
                return;
...
FunctionDialog* ApplicationWindow::showFunctionDialog(Graph *g, int curve)
{
    ...
    fd->setWindowTitle(tr("Analysis - Edit function"));
...
void ApplicationWindow::pixelLineProfile()
{
        ...
        int res = QInputDialog::getInteger(
            tr("Analysis - Set the number of pixels to average"), tr("Number of averaged pixels"),1, 1, 2000, 2,
                        &ok, this );
        if ( !ok )
                return;
...
void ApplicationWindow::addLayer()
{
        ...
        switch(QMessageBox::information(this,
                tr("Analysis - Guess best origin for the new layer?"),
...
void ApplicationWindow::fitLinear()
{
        ...
        if (w->isA("MultiLayer"))
                ...
                int cols = lst.size();
                if (!cols){
            QMessageBox::warning(this, tr("Analysis - Column selection error"), tr("Please select a 'Y' column first!"));
                        return;
                }
...
ApplicationWindow* ApplicationWindow::importOPJ(const QString& filename, bool factorySettings, bool newProject)
{
    if (filename.endsWith(".opj", Qt::CaseInsensitive) || filename.endsWith(".ogg", Qt::CaseInsensitive))
    {
        ...
        app->setWindowTitle("Analysis - " + filename);
                app->restoreApplicationGeometry();
...
void ApplicationWindow::fitMultiPeak(int profile)
{
        ...
        QMessageBox::warning(this,tr("Analysis - Warning"),
                ...
                bool ok;
        int peaks = QInputDialog::getInteger(tr("Analysis - Enter the number of peaks"),
        ...
        foreach(str, args){
            ...
            QMessageBox::critical(this, tr("Analysis - Error"),
                ...
                else if (str == "-v" || str == "--version")
                {
                        ...
                        #ifdef Q_OS_WIN
                hide();
                                QMessageBox::information(this, tr("Analysis") + " - " + tr("Version"), s);
                        #else
                ...
                else if (str == "-h" || str == "--help")
                {
                        ...
                        #ifdef Q_OS_WIN
                hide();
                                QMessageBox::information(this, tr("Analysis") + " - " + tr("Help"), s);
                        #else
                                std::wcout << s.toStdWString();
                        #endif
                        exit(0);
...
{
    saved = true;
    QMessageBox::critical(this, tr("Analysis - Demo Version"),
...
void ApplicationWindow::saveFolder(Folder *folder, const QString& fn, bool compress)
{
        ...
        {// make byte-copy of current file so that there's always a copy of the data on disk
                while (!f.open(QIODevice::ReadOnly)){
                        if (f.isOpen())
                                f.close();
            int choice = QMessageBox::warning(this, tr("Analysis - File backup error"),
        ...
        if ( !f.open( QIODevice::WriteOnly ) ){
        QMessageBox::about(this, tr("Analysis - File save error"), tr("The file: <br><b>%1</b> is opened in read-only mode").arg(fn));
                return;
        }
        QApplication::setOverrideCursor(QCursor(Qt::WaitCursor));
...
bool ApplicationWindow::deleteFolder(Folder *f)
{
    ...
    if (confirmCloseFolder && QMessageBox::information(this, tr("Analysis - Delete folder?"),
                ...
                bool folderNames, bool caseSensitive, bool partialMatch, bool subfolders)
{
    ...
    QMessageBox::warning(this, tr("Analysis - No match found"),
...
void ApplicationWindow::dropFolderItems(Q3ListViewItem *dest)
{
        ...
        foreach(it, draggedItems){
                if (it->rtti() == FolderListItem::RTTI){
                        ...
                        if (dest_f == f){
                QMessageBox::critical(this, "Analysis - Error", tr("Cannot move an object to itself!"));
                                return;
                        }

                        if (((FolderListItem *)dest)->isChildOf(src)){
                QMessageBox::critical(this,"Analysis - Error",tr("Cannot move a parent folder into a child folder!"));
                                draggedItems.clear();
                        ...
                        if (subfolders.contains(f->objectName())){
                                QMessageBox::critical(this, tr("Analysis") +" - " + tr("Skipped moving folder"),
...
bool ApplicationWindow::copyFolder(Folder *src, Folder *dest)
{
    if (!src || !dest)
        ...
        if (dest->subfolders().contains(src->objectName())){
                QMessageBox::critical(this, tr("Analysis") + " - " + tr("Error"),
...
{
    int choice = QMessageBox::question(this, tr("Analysis"),
...
{
        if (error){
        QMessageBox::warning(this, tr("Analysis - HTTP get version file"),
        ...
        if (version_buffer.open(IO_ReadOnly))
        {
                ...
                {
            if(QMessageBox::question(this, tr("Analysis - Updates Available"),
                ...
                {
            QMessageBox::information(this, tr("Analysis - No Updates Available"),
...
void ApplicationWindow::clearTable()
{
    ...
    if (QMessageBox::question(this, tr("Analysis - Warning"),
...
void ApplicationWindow::goToRow()
{
        ...
        if (w->inherits("Table") || w->isA("Matrix")){
                bool ok;
        int row = QInputDialog::getInteger(this, tr("Analysis - Enter row number"), tr("Row"),
...
void ApplicationWindow::goToColumn()
{
        ...
        if (w->inherits("Table") || w->isA("Matrix")){
                bool ok;
        int col = QInputDialog::getInteger(this, tr("Analysis - Enter column number"), tr("Column"),
                                1, 0, 1000000, 1, &ok, windowFlags() & ~Qt::WindowContextHelpButtonHint & ~Qt::WindowMinMaxButtonsHint );
                if ( !ok )
                        return;
...
bool ApplicationWindow::validFor2DPlot(Table *table)
{
        ...
        QMessageBox::warning(this, tr("Analysis - Error"), tr("Please select a Y column to plot!"));
            return false;
        } else if (table->numCols()<2) {
        QMessageBox::critical(this, tr("Analysis - Error"),tr("You need at least two columns for this operation!"));
                return false;
        } else if (table->noXColumn()) {
        QMessageBox::critical(this, tr("Analysis - Error"), tr("Please set a default X column for this table, first!"));
                return false;
        }
        return true;
}
...
{
        if (table->numCols()<2){
        QMessageBox::critical(0,tr("Analysis - Error"),tr("You need at least two columns for this operation!"));
                return false;
        }
        if (table->selectedColumn() < 0 || table->colPlotDesignation(table->selectedColumn()) != Table::Z){
        QMessageBox::critical(0,tr("Analysis - Error"),tr("Please select a Z column for this operation!"));
                return false;
        }
        if (table->noXColumn()){
        QMessageBox::critical(0,tr("Analysis - Error"),tr("You need to define a X column first!"));
                return false;
        }
        if (table->noYColumn()){
        QMessageBox::critical(0,tr("Analysis - Error"),tr("You need to define a Y column first!"));
                return false;
        }
        return true;
}
...
void ApplicationWindow::saveFitFunctions(const QStringList& lst)
{
    ...
    if (QMessageBox::Ok != QMessageBox::information(this, tr("Analysis") + " - " + tr("Import fit models"), explain,
