...
double Matrix::determinant()
{
	...
	if (!A || !p){
		...
		QMessageBox::critical((ApplicationWindow *)applicationWindow(),
				tr("Analysis") + " - " + tr("Memory Allocation Error"),
...
void Matrix::pasteSelection()
{
	...
	if (!clipboardBuffer){
		QMessageBox::critical(this, tr("Analysis") + " - " + tr("Memory Allocation Error"),
		tr("Not enough memory, operation aborted!"));
		QApplication::restoreOverrideCursor();
		return;
	}
...
double** Matrix::allocateMatrixData(int rows, int columns)
{
	...
	if(!data){
		QMessageBox::critical(0, tr("Analysis") + " - " + tr("Memory Allocation Error"),
		tr("Not enough memory, operation aborted!"));
		return NULL;
	}
...
bool Matrix::ignoreUndo()
{
	...
	return (QMessageBox::Yes == QMessageBox::warning((ApplicationWindow *)applicationWindow(),
		tr("Analysis") + " - " + tr("Warning"), msg, QMessageBox::Yes, QMessageBox::Cancel));
}
...
double* Matrix::initWorkspace(int size)
{
	...
	if (!d_workspace)
		QMessageBox::critical((ApplicationWindow *)applicationWindow(), tr("Analysis") + " - " + tr("Memory Allocation Error"),
		tr("Not enough memory, operation aborted!"));
