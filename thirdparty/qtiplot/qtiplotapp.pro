...
contains(SCRIPTING_LANGS, Python) {
  ...
  HEADERS += qtiplot/src/PythonScript.h qtiplot/src/PythonScripting.h
  SOURCES += qtiplot/src/PythonScript.cpp qtiplot/src/PythonScripting.cpp
  ...
  unix {
    ...
    system($$system(python python-sipcmd.py) -c $${SIP_DIR} qtiplot/src/qti.sip)
  }

  win32 {
    INCLUDEPATH += $$system(call python-includepath.py)
    LIBS        += $$system(call python-libs-win.py)
    system($$system(call python-sipcmd.py) -c $${SIP_DIR} qtiplot/src/qti.sip)
  }
