...
//	saveGLState();
    ...
    else if (style_ == FRAME_2D)
    {
        autoDecorateExposedAxis(axes[frameAxes_[0]], false);
        autoDecorateExposedAxis(axes[frameAxes_[1]], true);
    }
...
{ 
	...
	switch (s)
	{
		...
		case FRAME:
			{
				...
				if (!autoDecoration())
				{
                    ...
                    // Modified for STA: track the indices of the axes so that we can
                    // decorate them later.
                    frameAxes_[0] = frame_1;
                    frameAxes_[1] = frame_2;
                    frameAxes_[2] = frame_3;
                }
			}
            break;
        ...
        case FRAME_2D:
            {
                // Modified for STA: New coordinate style for 2D plotting.
                for (unsigned i=0; i!=axes.size(); ++i)
                    detach (&axes[i]);
                if (!autoDecoration())
                {
