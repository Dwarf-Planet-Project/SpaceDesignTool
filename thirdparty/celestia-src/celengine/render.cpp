...
public:
    ...
    void sample(double t, const Point3d& p, const Vec3d& v)
    {
...
public:
    ...
    void sample(double t, const Point3d& p, const Vec3d& v)
    {
        ...
        samp.position = Eigen::Vector3d(p.x, p.y, p.z);
        samp.velocity = Eigen::Vector3d(v.x, v.y, v.z);
        samp.t = t;
...
void renderOrbitColor(const Body *body, bool selected, float opacity)
{
    ...
    // STA only: Don't use a different color for orbits of the selected object
    selected = false;
                           ...
                           float farDist)
{
    ...
    if (orbit->isPeriodic())
    {
        ...
        if (startTime < cachedOrbit->startTime())
        {
            ...
            do {
                Point3d p = orbit->positionAtTime(orbitStartTime);
                Vec3d v = orbit->velocityAtTime(orbitStartTime);

                CurvePlotSample sample;
                sample.t = orbitStartTime;
                sample.position = Eigen::Vector3d(p.x, p.y, p.z);
                sample.velocity = Eigen::Vector3d(v.x, v.y, v.z);
        ...
        else if (endTime > cachedOrbit->endTime())
        {
            ...
            do {
                Point3d p = orbit->positionAtTime(orbitEndTime);
                Vec3d v = orbit->velocityAtTime(orbitEndTime);
    ...
    Eigen::Transform3d eigen_modelview;
    eigen_modelview.matrix() = Eigen::Matrix4d(&modelview[0][0]);

    Eigen::Vector3d viewFrustumPlaneNormals[4];
    for (int i = 0; i < 4; i++)
    {
        Vec3f n = frustum.getPlane(i).normal;
        viewFrustumPlaneNormals[i] = Eigen::Vector3d(n.x, n.y, n.z);
    }

    if (orbit->isPeriodic())
    {
        cachedOrbit->render(eigen_modelview,
...

    {
        ...
        for (int interval = 0; interval < nIntervals; interval++)
        {
            ...
            glDepthRange(1.0f - (float) (interval + 1) * intervalSize,
                         1.0f - (float) interval * intervalSize - 0.01f);
                            ...
                            float farPlaneDistance)
{
    ...
    if (discSizeInPixels > 1 && body.hasVisibleGeometry())
    {
        ...
        if (beginning == -numeric_limits<double>::infinity())
            beginning = astro::J2000;
    ...
    for (unsigned int i = 0; i < nChildren; i++)
    {
             ...
             (orbitVis == Body::UseClassVisibility && (body->getOrbitClassification() & orbitMask) != 0)))
        {
            ...
            if (orbitRadiusInPixels > minOrbitSize)
            {
                ...
                path.origin = Point3d(relOrigin.x, relOrigin.y, relOrigin.z);
        ...
        ((orbitMask & Body::Stellar) != 0 || highlightObject.star() == &star))
    {
        ...
        if (star.getOrbit() != NULL)
        {
            ...
            if (orbitRadiusInPixels > minOrbitSize)
            {
                ...
                path.centerZ = origf * viewMatZ;
                path.radius = (float) boundingRadius;
                path.origin = Point3d(orbitOrigin.x, orbitOrigin.y, orbitOrigin.z);
                path.opacity = sizeFade(orbitRadiusInPixels, minOrbitSize, 2.0f);
                orbitPathList.push_back(path);
            }
        }
