...
static const double MinSampleInterval = 1.0 / 1440.0 / 600.0;
static const double MaxSampleInterval = 50.0;
//static const double SampleThresholdAngle = 2.0;
static const double SampleThresholdAngle = 2.0 / 600.0;
                                                   ...
                                                   OrbitSampleProc& proc) const
{
    ...
    {
        Vec3d v;
        Vec3d p(samples[i].x, samples[i].y, samples[i].z);
        ...
        else if (i == 0)
        {
            ...
            v = (Vec3d(samples[i + 1].x, samples[i + 1].y, samples[i + 1].z) - p) / dt;
        }
        else if (i == samples.size() - 1)
        {
            double dt = samples[i].t - samples[i - 1].t;
            v = (p - Vec3d(samples[i - 1].x, samples[i - 1].y, samples[i - 1].z)) / dt;
        }
        else
        {
            double dt0 = samples[i + 1].t - samples[i].t;
            Vec3d v0 = (Vec3d(samples[i + 1].x, samples[i + 1].y, samples[i + 1].z) - p) / dt0;
            double dt1 = samples[i].t - samples[i - 1].t;
            Vec3d v1 = (p - Vec3d(samples[i - 1].x, samples[i - 1].y, samples[i - 1].z)) / dt1;
            v = (v0 + v1) * 0.5;
        }

        proc.sample(samples[i].t, Point3d(p.x, p.z, -p.y), Vec3d(v.x, v.z, -v.y));
    }
    ...
    proc.sample(current, positionAtTime(current), velocityAtTime(current));
                                                       ...
                                                       OrbitSampleProc& proc) const
{
    ...
    {
        proc.sample(iter->t,
                    Point3d(iter->position.x, iter->position.z, -iter->position.y),
                    Vec3d(iter->velocity.x, iter->velocity.z, -iter->velocity.y));
    }
...
template <typename T> SampledOrbit<T>* LoadSampledOrbit(const string& filename, TrajectoryInterpolation interpolation, double baseJD, T)
{
...
template <typename T> SampledOrbitXYZV<T>* LoadSampledOrbitXYZV(const string& filename, TrajectoryInterpolation interpolation, double baseJD, T)
{
    ifstream in(filename.c_str());
    if (!in.good())
        return NULL;
 ...
 *  baseJD is a date that will be added to each date in the file. By default it is zero. Setting
 *  it to some other value can be useful if the times in the file are offsets from J2000 (MJD)
 *  instead of normal Julian dates.
 */
Orbit* LoadSampledTrajectorySinglePrec(const string& filename, TrajectoryInterpolation interpolation, double baseJD)
{
    return LoadSampledOrbit(filename, interpolation, baseJD, 0.0f);
}
 ...
 *  instead of normal Julian dates.
 */
Orbit* LoadSampledTrajectoryDoublePrec(const string& filename, TrajectoryInterpolation interpolation, double baseJD)
{
    return LoadSampledOrbit(filename, interpolation, baseJD, 0.0);
}
 ...
 *  instead of normal Julian dates.
 */
Orbit* LoadXYZVTrajectorySinglePrec(const string& filename, TrajectoryInterpolation interpolation, double baseJD)
{
    return LoadSampledOrbitXYZV(filename, interpolation, baseJD, 0.0f);
}
 ...
 *  instead of normal Julian dates.
 */
Orbit* LoadXYZVTrajectoryDoublePrec(const string& filename, TrajectoryInterpolation interpolation, double baseJD)
{
    return LoadSampledOrbitXYZV(filename, interpolation, baseJD, 0.0);
}
