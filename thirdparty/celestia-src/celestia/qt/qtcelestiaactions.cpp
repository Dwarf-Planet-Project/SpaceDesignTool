    ...
    appCore(_appCore)
{
    ...
    // Navigation actions
    centerAction = new QAction(QString("Ctr"), this);
    centerAction->setToolTip(tr("Center selection"));
    connect(centerAction, SIGNAL(triggered()), this, SLOT(slotCenter()));   
    followAction = new QAction(QString("F"), this);
    followAction->setToolTip(tr("Follow selection"));
    connect(followAction, SIGNAL(triggered()), this, SLOT(slotFollow()));   
    gotoAction = new QAction(QString("G"), this);
    gotoAction->setToolTip(tr("Go to selection"));
    connect(gotoAction, SIGNAL(triggered()), this, SLOT(slotGoto()));   
    fastGotoAction = new QAction(QString("Z"), this);
    fastGotoAction->setToolTip(tr("Zoom to"));
    connect(fastGotoAction, SIGNAL(triggered()), this, SLOT(slotFastGoto()));   
    bodyFixedAction = new QAction(QString("Fix"), this);
    bodyFixedAction->setToolTip(tr("Body fixed frame of selection"));
    connect(bodyFixedAction, SIGNAL(triggered()), this, SLOT(slotBodyFixed()));   
...
void CelestiaActions::slotCenter()
{
    appCore->getSimulation()->centerSelection();
}


void CelestiaActions::slotFollow()
{
    appCore->getSimulation()->follow();
}


void CelestiaActions::slotBodyFixed()
{
    appCore->getSimulation()->geosynchronousFollow();
}


void CelestiaActions::slotGoto()
{
...
void CelestiaActions::slotFastGoto()
{
    Simulation* sim = appCore->getSimulation();
    if (sim->getFrame()->getCoordinateSystem() == ObserverFrame::Universal)
        sim->follow();
    sim->gotoSelection(0.1, Vec3f(0, 1, 0), ObserverFrame::ObserverLocal);    
}
