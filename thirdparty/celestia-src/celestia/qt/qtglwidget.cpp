...
void CelestiaGlWidget::initializeGL()
{
    ...
    if (glewErr != GLEW_OK)
    {
        ...
        QMessageBox::critical(NULL, "OpenGL Error", reinterpret_cast<const char*>(glewGetErrorString(glewErr)));
    }
...
#if 0
    // Modified for STA: Don't use saved Celestia renderer settings.
    // TODO: these settings shouldn't modified somewhere else; only the GL settings should be
    // modified here.
