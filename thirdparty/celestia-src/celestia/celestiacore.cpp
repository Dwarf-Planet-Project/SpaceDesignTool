...
// Patched by Guillermo on June 9th 2009
...
void CelestiaCore::mouseMove(float dx, float dy, int modifiers)
{
    ...
    if ((modifiers & (LeftButton | RightButton)) != 0)
    {
        ...
        {
            // STA NOTE: Right and left mouse functions reversed w.r.t. standard
            // Celestia interface.
...
void CelestiaCore::resize(unsigned int w, unsigned int h)
{
    if (h == 0)
	h = 1;
...
bool CelestiaCore::viewUpdateRequired() const
{
    ...
    for (list<View*>::const_iterator iter = views.begin(); iter != views.end(); iter++)
    {
        View* v = *iter;
        if (v->observer->getAngularVelocity().length() > 1.0e-10 ||
            v->observer->getVelocity().length() > 1.0e-12 ||
            v->observer->getMode() == Observer::Travelling)
        {
...
{
#if 0
    // Modified for STA: don't set the render flags here, because they can
    // override STA's settings. Propagate this change to Celestia.
    ...
    // Next line patched by Guillermo to take away the console log
