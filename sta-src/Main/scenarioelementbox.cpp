...
static QByteArray spaceVehicleWithTrajectoryFragment(const QString& name, const char* vehicleType)
{
    ...
    else if (name == "SmallGEO")
        loiteringDefault = myMissionDefaults.MissionsDefaults_SmallGEO();
    else if (name == "PROBA-1")
        loiteringDefault = myMissionDefaults.MissionsDefaults_PROBA1();
    else if (name == "PROBA-2")
        loiteringDefault = myMissionDefaults.MissionsDefaults_PROBA2();

    QSharedPointer<ScenarioSCSystemType> platform(myMissionDefaults.MissionsDefaults_GenericPlatform());
    QSharedPointer<ScenarioTransmitterPayloadType> transmitter(myMissionDefaults.MissionsDefaults_GenericTransmitter());
    QSharedPointer<ScenarioReceiverPayloadType> receiver(myMissionDefaults.MissionsDefaults_GenericReceiver());
    sc->setSystem(platform);

    sc->SCMission()->TrajectoryPlan()->AbstractTrajectory().append(QSharedPointer<ScenarioAbstractTrajectoryType>(&loiteringDefault));
    sc->SCMission()->PayloadSet()->AbstractPayload().append(transmitter);
    sc->SCMission()->PayloadSet()->AbstractPayload().append(receiver);
...
static QByteArray rendezVousManeouvresVFragment(const char* name)
{
    MissionsDefaults myMissionDefaults;
    ScenarioRendezVousManoeuvreType myRendezVousManeouvreSet = myMissionDefaults.MissionsDefaults_GENERIC_RENDEZVOUS();
    myRendezVousManeouvreSet.InitialPosition()->setAbstract6DOFPosition(QSharedPointer<ScenarioStateVectorType>(new ScenarioStateVectorType()));
    myRendezVousManeouvreSet.InitialAttitude()->setAbstract6DOFAttitude(QSharedPointer<ScenarioEulerType>(new ScenarioEulerType()));

    QDomDocument doc;
    return fragmentText(CreateRendezVousManoeuvreElement(&myRendezVousManeouvreSet, doc)).toUtf8();
}
        ...
        QWidget(parent)
{
    ...
    QTreeWidgetItem* rendezVousManeouvreSetItem      = new QTreeWidgetItem(maneuversItem);
    rendezVousManeouvreSetItem->setText(0, tr("RvD Set"));
    rendezVousManeouvreSetItem->setIcon(0, QIcon(":/icons/engine.png"));
    setDragAndDropInfo(rendezVousManeouvreSetItem,
                       MISSION_ARC_MIME_TYPE,
                       rendezVousManeouvresVFragment("RvD 1"));



    ///////////////////// Creating now the mission arcs: ascent, loitering, fly-by, re-entry
    ...
    addESASatelliteItem(ESASatellitesItem, "PROBA-1");
    addESASatelliteItem(ESASatellitesItem, "PROBA-2");
